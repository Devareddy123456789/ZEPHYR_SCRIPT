"""
Zephyr Squad Cloud Migration Script - FINAL FIXED VERSION

FIXES ALL ISSUES:
1. Groups CSV rows by ExecutionId (not by row)
2. Creates ONE execution per ExecutionId
3. Creates test steps only ONCE per Issue Key
4. Generates failure CSV for retry
5. Supports retry mode (process only failed issues)
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"      ❌ Zephyr error, giving up: {e}")
                return None
        except Exception as e:
...
def get_user_account_id(cfg: Config, name: str, cache: Dict[str, Optional[str]],
                       user_mapping: Dict[str, str], all_users: List[Dict[str, Any]],
...
def delete_execution(cfg: Config, execution_id: str, issue_id: str) -> bool:
    ...
    # Ensure issue_id is string for query parameter
    params = {"issueId": str(issue_id)}
    
    print(f"      🗑️  Attempting to DELETE execution {execution_id} (issueId={issue_id})...")
    resp = zephyr_request(cfg, "DELETE", path, params=params)
    
    if not resp:
        print(f"      ❌ DELETE failed: No response from API")
        return False
    
    if resp.status_code in (200, 204):
        print(f"      ✅ DELETE successful - UNEXECUTED execution removed")
        return True
    else:
...
def load_executions_from_csv(csv_path: str, issue_prefix: Optional[str] = None,
                             retry_issue_keys: Optional[Set[str]] = None) -> Dict[str, Dict[str, Any]]:
    """
    Load CSV and group by ExecutionId to avoid duplicates.
    Returns: {execution_id: {execution_data, steps: [...]}}
    """
    ...
    with open(csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        for raw in reader:
            ...
            if not issue_key:
                continue
            if issue_prefix and not issue_key.startswith(issue_prefix):
                continue
            if retry_issue_keys and issue_key not in retry_issue_keys:
                continue

            execution_id = pick(raw, "executionid", "execution id", default="")
            
            # Create unique key: execution_id if available, else issue_key + cycle + folder
            if execution_id:
...
def generate_failure_report(failed_items: List[Dict[str, Any]], output_path: str = "failed_executions.csv"):
    if not failed_items:
...
def migrate_executions(csv_path: str, cfg: Config, target_project_key: str,
                      user_mapping_path: Optional[str] = None, issue_prefix: Optional[str] = None,
                      retry_csv_path: Optional[str] = None) -> None:
    print("=" * 70)
    print("ZEPHYR SQUAD – EXECUTION MIGRATION (FINAL FIXED)")
    print("=" * 70)
    print(f"CSV:     {csv_path}")
    print(f"Project: {target_project_key}")
    if retry_csv_path:
        print(f"RETRY MODE: Processing only failed issues from {retry_csv_path}")
    print("=" * 70)

    # Load retry issue keys if in retry mode
    retry_issue_keys: Optional[Set[str]] = None
    if retry_csv_path:
        retry_issue_keys = set()
        with open(retry_csv_path, newline="", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            for row in reader:
                issue_key = row.get("issue_key", "")
                if issue_key:
                    retry_issue_keys.add(issue_key)
        print(f"✅ Loaded {len(retry_issue_keys)} failed issue keys for retry\n")
    ...
    # Load executions (grouped by ExecutionId)
    executions = load_executions_from_csv(csv_path, issue_prefix, retry_issue_keys)
    ...
    for idx, (exec_id, exec_data) in enumerate(executions.items(), start=1):
        ...
        if exec_data["executed_by"]:
            executed_by_id = get_user_account_id(cfg, exec_data["executed_by"], user_cache,
                                                user_mapping, all_users, verbose=False)
        ...
        if ok:
            ...
            delete_success = delete_execution(cfg, new_exec_id, issue_id)
            
            failed += 1
            failure_data = {
                **exec_data, 
                "execution_id": new_exec_id,
                "failure_reason": "Could not execute execution (deleted)" if delete_success else "Could not execute execution (DELETE FAILED - manual cleanup needed)"
            }
            failed_items.append(failure_data)
            
            if not delete_success:
                print(f"   🚨 WARNING: Could not delete UNEXECUTED execution {new_exec_id}")
                print(f"   🚨 Manual cleanup required in Zephyr UI!")
...
def main():
    parser = argparse.ArgumentParser(description="Zephyr Squad migration (FINAL FIXED)")
    parser.add_argument("--csv", required=True, help="Path to CSV")
    parser.add_argument("--config", required=True, help="Path to config")
    parser.add_argument("--target-project-key", required=True, help="Target project")
    parser.add_argument("--issue-prefix", help="Only process issues with this prefix")
    parser.add_argument("--user-mapping", help="Path to user mapping JSON")
    parser.add_argument("--retry-from-failures", help="Path to failed_executions.csv to retry only failed issues")
    ...
    migrate_executions(args.csv, cfg, args.target_project_key,
                      ...
                      retry_csv_path=args.retry_from_failures)
